...
export const data = new SlashCommandBuilder()
  .setName("cost")
  .setDescription(
    "Estimate the point value of a ship, fort... based on their stats, using UDC and SimCost algorithms."
  )
  ...
  .addStringOption((option) =>
    option
      .setName("speed")
      .setDescription(
        'Letters (S, L, D, T) with or without a + sign in between (ex: "S+L" or "sSs")'
      )
      .setRequired(true)
      .setMaxLength(5)
  )
  .addStringOption((option) =>
    option
      .setName("cannons")
      .setDescription(
        'Dice (1 to 6) and range (S or L) with or without a space in between (ex: "4s 3L2l4S")'
      )
      .setRequired(true)
      .setMaxLength(30)
  );
...
export async function execute(interaction) {
  ...
  const cannons = sanitize(
    interaction.options.getString("cannons")
  ).toUpperCase();

...
function outputSimulatedCost(masts, cargo, speed, cannons) {
  ...
  return `${emojis.masts} ${masts} ${emojis.cargo} ${cargo} ${
    emojis.speed
  } ${speed} ${emojis.cannon} ${cannons} = :coin: ${total
    .toFixed(1)
    .replace(".0", "")}`;
}
...
export default function cost(masts, cargo, speed, cannons) {
  ...
  if (!speedRegex.test(speed)) {
    errors.push(
      '`speed` should be letters (S, L, D, T) with or without a + sign in between (ex: "S+L" or "sSs")'
    );
  }
  if (!cannonsRegex.test(cannons)) {
    errors.push(
      '`cannons` should be dice (1 to 6) and range (S or L) with or without a space in between (ex: "4S3L2L4S" or "4s 3l 2l 4s")'
    );
  }
  ...
  const _speed = speed.replace(/\+/g, "");
  const _cannons = cannons.replace(/ /g, "").match(/.{2}/g); // cuts the string into 2-char-long segments
  ...
  // Cannons
  if (_cannons) {
    cannons_points = _cannons.reduce((total, [cannon_unit, cannon_range]) => {
      // 6 5 4 => free
      if (cannon_unit > 3) {
        return total;
      }
  ...
  const udc_calc = outputSimulatedCost(
    masts_points,
    cargo_points,
    speed_points,
    cannons_points
  ...
  cannons_points = round(
    simcost_cannons_unit * 0.3 + simcost_cannons_range * 0.2
  );

  const simcost_calc = outputSimulatedCost(
    masts_points,
    cargo_points,
    speed_points,
    cannons_points
